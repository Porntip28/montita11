...
//==========================JOIN ตารางที่ต้องการเเสดงผล==============
  ...
  $query1 = $db->prepare("SELECT * FROM order_food 
  INNER JOIN detail_order_food
  ...
  <table class="table table-striped table-bordered cart">
    ...
    <tbody>
      ...
      <tr>
          <td><?=++$i;?></td>
          <td>
            <input type="text" name="food_id[<?= $key ?>]" value="<?= $val->food_id; ?>" > <!-- ส่งค่า$key เเละ pill_idไปเพื่อจะอัพเดท-->
          </td>
          <td><?=$val->food_name?></td>
