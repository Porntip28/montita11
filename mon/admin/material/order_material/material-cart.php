...
<br><br>
<p><a href="?file=material/material-part/index" class="btn btn-info" role="button">เลือกอุปกรณ์เพิ่ม</a>
  <table class="table table-striped table-bordered cart">
    ...
    <tbody>
      ...
      foreach($_SESSION['cart'] as $key=> $val):   ?>              <!--ให้$_SESSION['cart']ชี้ไปที่ตัวเเปร$key=> $val   ($key แทน ,$valแทนจ ำนวน )-->
      <?php $price_all = $val['item']->price * $val['amount'];    //รวมราคาต่อชิ้น
            $price_total=$price_total+$price_all;                 //รวมราคาทั้งหมด
       ?>

      <tr >
        <td><?=++$i;?></td>                                      <!--ส่วนของการเเสดงรายการที่เลือก -->
