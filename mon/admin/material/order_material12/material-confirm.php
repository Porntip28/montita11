...
if(isset($_SESSION['cart'])){
      $query = $db->prepare("INSERT INTO order_material
         (date_order_material, order_status, user_id)
          VALUES (:date_order_material, :order_status, :user_id)");
          $result = $query->execute([
            'date_order_material'=>date('Y-m-d H:i:s'),
  ...
  if($result){
    ...
    foreach($_SESSION['cart'] as $key => $item){
      $query = $db->prepare("INSERT INTO detail_order_material
        (order_material_id, material_id, price, quantity)
       VALUES ( :order_material_id, :material_id, :price, :quantity)");

      $result = $query->execute([
        'order_material_id' => $lastId,
        'material_id' => $item['item']->material_id,
      ...
      echo "<script>
            window.location = '?file=material/order_material/order_material_view&id=$lastId';
            </script>";
    }
}
?>
