...
if(isset($_SESSION['cart'])){
      $query = $db->prepare("INSERT INTO order_pill
         (date_order_pill, order_status, user_id)
          VALUES (:date_order_pill, :order_status, :user_id)");
          $result = $query->execute([
            'date_order_pill'=>date('Y-m-d H:i:s'),
  ...
  if($result){
    ...
    foreach($_SESSION['cart'] as $key => $item){
      $query = $db->prepare("INSERT INTO detail_order_pill
        (order_pill_id, pill_id, price, quantity)
       VALUES ( :order_pill_id, :pill_id, :price, :quantity)");

      $result = $query->execute([
        'order_pill_id' => $lastId,
        'pill_id' => $item['item']->pill_id,
      ...
      echo "<script>
            window.location = '?file=pill/order_pill/order_pill_view&id=$lastId';
            </script>";
    }
}
?>
